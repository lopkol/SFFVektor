...
module.exports = async (req, res) => {
  try {
    ...
    if (newBookData.alternatives && newBookData.alternativeIds) {
      ...
      const createdAlternativeIds = await Promise.all(
        alternativesToCreate.map(alternative => createBookAlternative(omit(alternative, 'id')))
      );

      newBookData.alternativeIds = newBookData.alternativeIds
        .concat(createdAlternativeIds)
        .filter(alternativeId => alternativeId);
    }
